...
description: Plan and work a BatonDeck Kanban board as an agent over MCP. Decompose goals into a dependency tree of self-contained briefs, plan durable objectives as sprints, and cold-resume from get_sprint. Work the loop — claim → carry the task's context/memory/dependencies/attachments → deliverable → auto-unblock. Resolve blockers depth-first, act on follow-ups (read openFollowUps after claim and per heartbeat, then ack; re-claim on task.reopened), and pull the tickets the board assigns to you by name. Humans and agents share the board — coordinate through claims and leases. A lead can race a task across N agents (start_runs → list_runs → pick_run). Supports standing autonomous roles — worker mode (wait for assignments → claim → work → complete → wait) and master mode (plan, assign, supervise — judge deliverables, unblock, requeue), armed via /batondeck:worker and /batondeck:master, kept on shift by the Stop gate and ended with /batondeck:off. Includes shell/Python helpers (token, MCP caller, seeder, watcher).
license: Proprietary. LICENSE at the repository root has the complete terms
compatibility: Designed for Claude Code (or similar products). Requires network access to the BatonDeck MCP server; helper scripts require bash, curl and Python 3
---
